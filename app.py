...
st.set_page_config(page_title="InfraTrack v4.8", page_icon="🚆", layout="wide") # Version updated
...
# --- TITOLO E HEADER ---
st.markdown("## 🚆 InfraTrack v4.8") # Version updated
...
if uploaded_file is not None:
    if not st.session_state.file_processed_success:
        with st.spinner('Caricamento e analisi completa del file in corso...'):
            try:
                # ... (Logica parsing e estrazione dati generali/date progetto/minuti_giorno identica a v4.7) ...
                ...
                def format_duration_from_xml(duration_str):
                     # ... (funzione durata identica a v4.7) ...
                ...
                for task in all_tasks:
                    # ... (Logica estrazione dati attività identica a v4.7, incluso calcolo slack) ...
                    ...
                    # Logica TUP/TUF (con accesso sicuro a DurataSecondi)
                    ...
                    if match:
                         ...
                         # --- CORREZIONE: Usa .get() nella comparazione ---
                         existing_duration_seconds = potential_milestones.get(tup_tuf_key, {}).get("DurataSecondi", -1) # Usa -1 se non esiste
                         # --- FINE CORREZIONE ---
                         ...
                         if tup_tuf_key not in potential_milestones:
                              ...
                              # Usiamo existing_duration_seconds ottenuto con .get()
                ...
                # Salvataggio dati TUP/TUF e All Tasks (Identico a v4.7)
                final_milestones_data = []
                for key in potential_milestones:
                     data = potential_milestones[key]
                     final_milestones_data.append({ # Usiamo .get() come in v4.7
            ...
            except KeyError as ke: # Cattura specificamente KeyError
                 st.error(f"Errore interno nell'elaborazione dei dati: Chiave mancante {ke}")
                 st.error("Questo indica un problema nella struttura dati attesa. Controlla la logica di estrazione TUP/TUF.")
    ...
    if st.session_state.file_processed_success:
        # --- Sezione 2: Analisi Preliminare (Identica a v4.7) ---
        st.markdown("---")
        st.markdown("#### 2. Analisi Preliminare")
        st.markdown("##### 📄 Informazioni Generali dell'Appalto")
        # ... (Visualizzazione dati generali e TUP/TUF + download identica a v4.7) ...
        ...
        # --- Sezione 3: Analisi Avanzata (Identica a v4.7) ---
        ...
        if all_tasks_df is not None and not all_tasks_df.empty:
            try:
                # ... (Logica filtro data e analisi criticità identica a v4.7) ...
