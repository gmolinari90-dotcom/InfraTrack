...
if current_file_to_process is not None:
    ...
    if st.session_state.get('file_processed_success', False):
        ...
        # --- [MODIFICATO v19.9] Sezione Istogrammi Risorse (Arrotondamento Standard) ---
        st.markdown("---")
                    ...
                    with st.spinner(f"Calcolo unità medie giornaliere ({selected_resource_type})..."):
                        ...
                        if filtered_work.empty:
                            ...
                            if selected_resource_type == 'Mezzi':
                                ...
                                # --- [CORREZIONE v19.10] Calcolo Unità Manodopera/Altro/Tutte ---
                                aggregated_daily_total = filtered_work.groupby('Date')['WorkHours'].sum().reset_index() # Somma Ore
