                            ...
                            if selected_resource_type in ['Mezzi', 'Altro']:
                                ...
                                if aggregation_level == 'Mensile':
                                     try:
                                        ...
                                        df_pivot_export = df_pivot_export.reindex(df_export_hist['Date'].unique()) # Forza ordinamento
            ...
            if all_tasks_df is None or all_tasks_df.empty:
                ...
                try:
                    with st.spinner(f"Calcolo attività critiche (Flessibilità <= {slack_threshold} giorni)..."):
                        ...
                        # Filtro per attività (non riepiloghi) e Flessibilità
                        tasks_df_crit = tasks_df_crit[
                            (tasks_df_crit['Summary'] == False) &
                            (tasks_df_crit['TotalSlackDays'] <= slack_threshold)
                        ]
