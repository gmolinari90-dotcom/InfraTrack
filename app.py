# --- v8.0 (Base Stabile + Selezione Date CORRETTA) ---
...
st.set_page_config(page_title="InfraTrack v8.0", page_icon="🚆", layout="wide") # Version updated

# --- CSS ---
st.markdown("""
<style>
    /* Stili CSS stabili (omesso per brevità) */
...
# --- TITOLO E HEADER ---
st.markdown("## 🚆 InfraTrack v8.0") # Version updated
...
if st.button("🔄", key="reset_button", help="Resetta l'analisi", disabled=not st.session_state.file_processed_success):
    ...
    keys_to_reset = ['uploaded_file_state', 'project_name', 'formatted_cost','df_milestones_display', 'debug_raw_text', 'project_start_date','project_finish_date', 'minutes_per_day', 'all_tasks_data'] # No slider_value
        ...
        with st.spinner('Caricamento e analisi completa del file in corso...'):
            try:
                ...
                for key in potential_milestones:
                     ...
                     final_milestones_data.append({...}) # Omissis .get() logic
