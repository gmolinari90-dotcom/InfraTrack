...
st.set_page_config(page_title="InfraTrack v3.0", page_icon="🚆", layout="wide") # Version updated
...
# --- TITOLO E HEADER ---
st.markdown("## 🚆 InfraTrack v3.0") # Version updated
...
if st.button("🔄", key="reset_button", help="Resetta l'analisi", disabled=not st.session_state.file_processed_success):
    ...
    # Resetta anche i dati salvati
    keys_to_reset = ['project_name', 'formatted_cost', 'df_milestones_display',
                     'debug_raw_text', 'project_start_date', 'project_finish_date',
                     'all_tasks_data'] # Aggiungeremo 'all_tasks_data'
...
if uploaded_file is not None:
        ...
        with st.spinner('Caricamento e analisi del file in corso...'):
            try:
                ...
                if task_uid_1 is not None:
                    ...
                    # Estrai date inizio/fine progetto dall'attività UID 1
                ...
                all_tasks = tree.findall('.//msp:Task', namespaces=ns) # Prendiamo tutte le attività
                ...
                # -- Estrazione Dati di Tutte le Attività (per analisi future) --
                # Salviamo i dati rilevanti di OGNI attività in una lista di dizionari
                all_tasks_data = []
                for task in all_tasks:
                    ...
                    wbs = task.findtext('msp:WBS', namespaces=ns) or "" # Estrai WBS
                    total_slack_str = task.findtext('msp:TotalSlack', namespaces=ns) or "0" # Estrai Slack (in minuti di default in XML)
                    ...
                    # Converti TotalSlack da minuti a giorni (assumendo 8 ore/giorno, 60 min/ora)
                ...
                # Salviamo i dati TUP/TUF in sessione
                ...
                # Salviamo TUTTI i dati delle attività in sessione come DataFrame Pandas
                st.session_state['all_tasks_data'] = pd.DataFrame(all_tasks_data)
            ...
            except etree.XMLSyntaxError as e: st.error(f"Errore Sintassi XML: {e}"); st.error("File malformato?"); st.session_state.file_processed_success = False; #... (omissis)
            except Exception as e: st.error(f"Errore Analisi: {e}"); st.error("Verifica file XML."); st.session_state.file_processed_success = False; #... (omissis)

    # --- VISUALIZZAZIONE DATI E NUOVA SEZIONE ANALISI ---
    if st.session_state.file_processed_success:
        ...
        if df_display is not None and not df_display.empty:
            ...
            output = BytesIO(); #... (omissis download excel)
        ...
        # --- NUOVA SEZIONE: 3. Analisi Avanzata ---
        st.markdown("---")
        st.markdown("#### 3. Analisi Avanzata")

        # Recupera le date di inizio/fine del progetto dalla sessione
        ...
        with col_date1:
            selected_start_date = st.date_input(
                "Data Inizio Analisi",
                value=default_start,
                min_value=default_start, # Imposta limite minimo
                max_value=default_finish, # Imposta limite massimo
                format="DD/MM/YYYY" # Formato europeo
            )
        with col_date2:
             # Assicura che la data fine non sia precedente alla data inizio selezionata
            min_end_date = selected_start_date + timedelta(days=1)
            # Ricalcola default_finish se min_end_date lo supera
            actual_default_finish = max(default_finish, min_end_date)

            selected_finish_date = st.date_input(
                "Data Fine Analisi",
                value=actual_default_finish,
                min_value=min_end_date, # Fine deve essere dopo inizio
                max_value=actual_default_finish + timedelta(days=10*365), # Limite max flessibile
                format="DD/MM/YYYY" # Formato europeo
            )

        # Mostra le date selezionate (per debug/conferma)
