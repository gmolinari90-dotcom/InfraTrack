...
st.set_page_config(page_title="InfraTrack v10.1", page_icon="🚆", layout="wide") # Version updated

# --- CSS ---
# ... (CSS Identico a v10.0 - omesso per brevità) ...
st.markdown("""
<style>
    /* ... */
...
# --- TITOLO E HEADER ---
st.markdown("## 🚆 InfraTrack v10.1") # Version updated
...
if current_file_to_process is not None:
        ...
        with st.spinner('Caricamento e analisi completa del file in corso...'):
            try:
                ...
                for task in all_tasks:
                    # ... (Estrazione dati base attività come v10.0) ...
                    ...
                    # --- CORREZIONE DEFINITIVA SYNTAXERROR ---
                    total_slack_days = 0
                    ...
                    # Logica TUP/TUF (identica a v10.0)
                ...
                # Salvataggio dati TUP/TUF (Identico a v10.0)
                ...
                # Salvataggio TUTTE le attività (Identico a v10.0)
                st.session_state['all_tasks_data'] = pd.DataFrame(all_tasks_data_list)

                # Estrazione Dati Temporizzati (Identico a v10.0)
                ...
                # Estrazione Dati Risorse (Identico a v10.0)
    ...
    if st.session_state.get('file_processed_success', False):
        # --- Sezione 2: Analisi Preliminare (Identica a v10.0) ---
        st.markdown("---"); st.markdown("#### 2. Analisi Preliminare"); st.markdown("##### 📄 Informazioni Generali dell'Appalto")
        # ... (Visualizzazione nome/costo + TUP/TUF + download identica a v10.0) ...
        ...
        # --- Sezione 3: Selezione Periodo e Analisi (Identica a v10.0) ---
        ...
        if baseline_cost_df is None or baseline_work_df is None or resources_df is None:
             st.error("Errore: Dati temporizzati o risorse mancanti.")
        else:
            try:
                # --- CURVA S (SIL) ---
                # ... (Logica Curva S identica a v10.0) ...
                ...
                if not filtered_cost.empty: #... (Grafico SIL + download) ...
                    monthly_cost = filtered_cost.set_index('Date').resample('ME')['Value'].sum().reset_index(); #...
                    fig_sil = px.line(monthly_cost, x='Mese', y='Costo Cumulato (€)', ...); st.plotly_chart(fig_sil, use_container_width=True)
                    output_sil = BytesIO(); #... (download) ...; st.download_button(label="Scarica Dati SIL (Excel)", ...)
                else: st.warning("Nessun dato costo baseline nel periodo.")

                # --- ISTOGRAMMI MANODOPERA E MEZZI ---
                # ... (Logica Istogrammi identica a v10.0) ...
                ...
                if not filtered_work.empty and not resources_df.empty:
                     ...
                     if not manodopera_df.empty: #... (Grafico Manodopera + download) ...
                         fig_manodopera = px.bar(manodopera_df, x='Mese', y='Value', ...); st.plotly_chart(fig_manodopera, use_container_width=True)
                     ...
                     if not mezzi_df.empty: #... (Grafico Mezzi + download) ...
                         fig_mezzi = px.bar(mezzi_df, x='Mese', y='Value', ...); st.plotly_chart(fig_mezzi, use_container_width=True)
                ...
                else: st.warning("Nessun dato lavoro baseline nel periodo o risorse non definite.")
        ...
        #else: # Questo else non serve perché gestito sopra
