...
st.set_page_config(page_title="InfraTrack v12.4", page_icon="🚆", layout="wide") # Version updated
...
# --- TITOLO E HEADER ---
st.markdown("## 🚆 InfraTrack v12.4") # Version updated
...
# --- FUNZIONE ESTRAZIONE DATI TEMPORIZZATI (Nome corretto) ---
@st.cache_data
    ...
    if not st.session_state.get('file_processed_success', False):
        with st.spinner('Caricamento e analisi completa del file in corso...'):
            try:
                ...
                # --- ESTRAZIONE DATI TEMPORIZZATI CON NOME CORRETTO ---
                assignments_node = tree.find('msp:Assignments', ns)
                
                # Cerca Costo Baseline (Type 9) o Fallback (Type 2) per SIL
                # USA IL NOME CORRETTO: extract_timephased_data_from_assignments
