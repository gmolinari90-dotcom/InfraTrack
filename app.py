...
st.set_page_config(page_title="InfraTrack v1.6", page_icon="🚆", layout="wide") # Version updated
...
<style>
    ...
    .stApp h1, .stApp h2, .stApp h3, .stApp h4, .stApp h5, .stApp h6, .stApp p, .stApp .stDataFrame, .stApp .stButton>button {
     ...
     .stApp .stMarkdown h4 { /* Titoli Sezione (1. Carica..., 2. Analisi...) */
         ...
         margin-top: 1rem; /* Aggiungi spazio sopra */
     }
     .stApp .stMarkdown h5 { /* Sottotitoli (Info Generali, Termini Utili) */
         font-size: 0.90rem !important;
         margin-bottom: 0.5rem;
         margin-top: 0.8rem; /* Aggiungi spazio sopra */
     }
...
# --- TITOLO E HEADER ---
st.markdown("## 🚆 InfraTrack v1.6") # Version updated
...
if uploaded_file is not None:
    st.markdown("---")
    # --- NUOVA STRUTTURA TITOLI ---
    st.markdown("#### 2. Analisi Preliminare") # Titolo Principale della Sezione
    ...
    with st.spinner('Caricamento e analisi del file in corso...'):
        try:
            ...
            # Rimuoviamo la linea divisoria qui per tenere tutto unito
            ...
            # --- Sottosezione: Termini Utili ---
            # Nome modificato e ora sotto "2. Analisi Preliminare"
            ...
            if final_milestones_data:
                ...
                with pd.ExcelWriter(output, engine='openpyxl') as writer:
                    df_display.to_excel(writer, index=False, sheet_name='TerminiUtili') # Nome sheet aggiornato
                excel_data = output.getvalue()
                st.download_button(
                    label="Scarica (Excel)",
                    data=excel_data,
                    file_name="termini_utili_TUP_TUF.xlsx", # Nome file aggiornato
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )

            else:
                st.warning("Nessun Termine Utile (TUP o TUF) trovato nel file.") # Testo aggiornato
...
#else:
    # Mostra messaggio iniziale solo se non c'è un file caricato
    # if 'uploaded_file_state' not in st.session_state or st.session_state.uploaded_file_state is None:
    #    st.info("Carica un file XML per iniziare l'analisi.")
    # pass
