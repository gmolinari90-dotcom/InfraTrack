# --- v6.0 (Base Stabile Ripristinata) ---
...
st.set_page_config(page_title="InfraTrack v6.0", page_icon="🚆", layout="wide") # Version updated

# --- CSS ---
st.markdown("""
<style>
    /* Stili CSS v2.15 (bottone sotto titolo) */
...
# --- TITOLO E HEADER ---
st.markdown("## 🚆 InfraTrack v6.0") # Version updated
...
if st.button("🔄", key="reset_button", help="Resetta l'analisi", disabled=not st.session_state.file_processed_success):
    ...
    # Rimuoviamo solo le chiavi relative all'analisi preliminare salvate fino a v2.15/v5.4
    keys_to_reset = ['uploaded_file_state', 'project_name', 'formatted_cost',
                     'df_milestones_display', 'debug_raw_text', 'project_start_date',
                     'project_finish_date', 'minutes_per_day', 'all_tasks_data'] # Include all_tasks_data se veniva già salvato
...
if uploaded_file is not None:
    # --- Elaborazione Dati (Logica stabile pre-Sezione 3) ---
    if not st.session_state.file_processed_success:
        with st.spinner('Caricamento e analisi completa del file in corso...'):
            try:
                ...
                # Estrazione minuti/giorno (essenziale per durate)
                minutes_per_day = 480
                ...
                if default_calendar is not None:
                     ...
                     if working_day is not None:
                          ...
                          for working_time in working_day.findall(".//msp:WorkingTime", namespaces=ns):
                               # ... (calcolo working_minutes omesso per brevità) ...
                ...
                # --- Estrazione TUP/TUF (Logica stabile v2.15/v4/v5 - SENZA errori indentazione/key) ---
                ...
                for task in all_tasks:
                    ...
                    if match:
                         ...
                         duration_formatted = format_duration_from_xml(duration_str_tup)
                         try:
                ...
                # Estrazione dati TUTTE le attività (necessario per analisi future)
                all_tasks_data_list = []
                for task in all_tasks:
                    uid = task.findtext('msp:UID', namespaces=ns)
                    if uid == '0': continue # Saltiamo la riga 0 ora che abbiamo già i dati generali
                    name = task.findtext('msp:Name', namespaces=ns) or ""; start_str = task.findtext('msp:Start', namespaces=ns); finish_str = task.findtext('msp:Finish', namespaces=ns)
    ...
    # --- VISUALIZZAZIONE DATI (Solo Analisi Preliminare) ---
    if st.session_state.file_processed_success:
        ...
        # --- Sezione 3: Placeholder ---
        st.markdown("---")
        st.markdown("#### 3. Analisi Avanzata")
        st.info("Funzionalità di analisi avanzata (selezione periodo, percorso critico, ecc.) da implementare.")
